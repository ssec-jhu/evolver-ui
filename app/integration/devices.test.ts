...
test("devices route renders the device list page", async ({ page }, testInfo) => {
  ...
  const deviceLink = await page.locator('table td a', { hasText: 'Test Evolver Device' });
...
test("device detail page and tab navigation", async ({ page }, testInfo) => {
  ...
  const breadcrumbDeviceName = await page.getByRole('link', { name: 'Test Evolver Device' });
